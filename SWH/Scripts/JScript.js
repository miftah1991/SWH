    ...
    $.getJSON(varURL, function (data) {
        ...
        $(varCombo).chosen().change(function () {
            if ($(this).val() == null) {
                ...
                $(varCombo + '_chosen').find('a:first').css("border-color", "darkblue");
            }
        });
    });
}
            ...
            if (isValid) {
                ...
                $.ajax(
                {
                    ...
                    success: function (results) {
                        if (results.Message == "Record Saved") {
                            var newID = results.EduID;
                            $('#' + varFrm).append("<input type='hidden' id='" + varPK + "' name='" + varPK + "' value='' />");
                            toastr["success"]("Recrod Save Successfully");
                            $('#' + varModal).modal('toggle');
                            $('#' + varGrid).jqxGrid('updatebounddata', 'data');
                        }
                        else
                            if (results.Message == "Record Updated") {
                                toastr["success"]("Recored Updated Successfully");
                                $('#' + varModal).modal('toggle');
                                $('#' + varGrid).jqxGrid('updatebounddata', 'data');
                            }
                            else {
...
function setComb(varCombo, varURL, varID, varValue, varTheme, varHeight, varWidth, varAuto) {
    ...
    $(varCombo).jqxComboBox({ source: dataAdapter, displayMember: varValue, valueMember: varID, width: varWidth, theme: varTheme, autoDropDownHeight: true });
}
